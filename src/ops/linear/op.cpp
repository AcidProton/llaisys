...
void linear(tensor_t out, tensor_t in, tensor_t weight, tensor_t bias, bool use_bias) {
    if(use_bias){
    ...
    std::byte* bias_data = use_bias?bias->data():nullptr;

    // always support cpu calculation
    if (weight->deviceType() == LLAISYS_DEVICE_CPU) {
        return cpu::linear(out->data(), in->data(), weight->data(), bias_data, weight->dtype(), use_bias, m, n, k);
    }
