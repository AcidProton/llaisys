...
template <typename T>
void linear_(T *out, const T *in, const T *weight, const T *bias, bool use_bias, size_t m, size_t n, size_t k) {
        ...
        if(use_bias){
            ...
            for(size_t idx_m=0; idx_m<m; idx_m++){
                for(size_t idx_n=0; idx_n<n; idx_n++){
                    ...
                    out[idx_m*n + idx_n] = llaisys::utils::cast<T>(ele);
                }
            }
        }
    } else {
...
void linear(std::byte *out, const std::byte *in, const std::byte *weight, const std::byte *bias, llaisysDataType_t type, bool use_bias, size_t m, size_t n, size_t k) {
    switch (type) {
    case LLAISYS_DTYPE_F32:
        return linear_(reinterpret_cast<float *>(out), reinterpret_cast<const float *>(in), reinterpret_cast<const float *>(weight), 
                reinterpret_cast<const float *>(bias), use_bias, m, n, k);
    case LLAISYS_DTYPE_BF16:
        return linear_(reinterpret_cast<llaisys::bf16_t *>(out), reinterpret_cast<const llaisys::bf16_t *>(in), reinterpret_cast<const llaisys::bf16_t *>(weight), 
                reinterpret_cast<const llaisys::bf16_t *>(bias), use_bias, m, n, k);
    case LLAISYS_DTYPE_F16:
        return linear_(reinterpret_cast<llaisys::fp16_t *>(out), reinterpret_cast<const llaisys::fp16_t *>(in), reinterpret_cast<const llaisys::fp16_t *>(weight), 
                reinterpret_cast<const llaisys::fp16_t *>(bias), use_bias, m, n, k);
    default:
